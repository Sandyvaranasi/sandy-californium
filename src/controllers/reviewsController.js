...
const createReviws = async function (req, res) {
    try {
        ...
        let {reviewedBy, rating, review,...a } = req.body;

        if(Object.keys(a).length!=0) return res.status(400).send({status:false,message:"invalid key"})
...
const updateReviews = async function (req, res) {
    try {
        ...
        let { review, rating, reviewedBy,...a } = data;
        ...
        let bookData = await bookModel.findOne({ _id: bookId, isDeleted: false }).select({__v:0});
